//
//  RCTDigitsAuthenticateManager.m
//  MyDigitsDemo
//
//  Created by Jean Lebrument on 26/10/15.
//  Copyright © 2015 Jean Lebrument. All rights reserved.
//

#import "RCTDigitsAuthenticateManager.h"
#import <DigitsKit/DigitsKit.h>

@implementation RCTDigitsAuthenticateManager
...
- (void)setFontToApparence:(DGTAppearance *)appearance
  ...
  if (fontName && fontSize && [fontName isKindOfClass:[NSString class]] && [fontSize isKindOfClass:[NSNumber class]]) {
...
RCT_EXPORT_METHOD(authenticateDigitsWithOptions:(NSDictionary *)options callback:(RCTResponseSenderBlock)callback) {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [[Digits sharedInstance] authenticateWithViewController:root configuration:configuration completion:^(DGTSession *session, NSError *error) {
      if (error) {
        callback(@[@{@"code": [NSNumber numberWithInteger:error.code], @"domain": error.domain},
                   [NSNull null]]);
      } else {
